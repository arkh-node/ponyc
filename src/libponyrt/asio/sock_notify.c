      ...
      )) != NULL)
    {
      ...
      switch(msg->flags)
      {
        ...
        case ASIO_CANCEL_SIGNAL:
        {
          ...
          // SignalNotify.disposed promises that by the time it fires the
          // handler no longer affects delivery — for the signal's last
          // subscriber that means the default disposition is already
          // restored. Hoisting this send above the teardown would falsify
          // that promise.
          ev->flags = ASIO_DISPOSABLE;
          pony_asio_event_send(ev, ASIO_DISPOSABLE, 0);
          break;
