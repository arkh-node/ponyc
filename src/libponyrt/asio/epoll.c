...
// docstrings state it and packages/signals/_test.pony asserts it (16 register,
// the 17th error-disposes) — and epoll.c, kqueue.c, and sock_notify.c must all
// use the same value. A backend-only change desyncs the platforms; changing all
// three still breaks the stdlib test and contradicts the docstrings.
    ...
    )) != NULL)
  {
    ...
    switch(msg->flags)
    {
      ...
      case ASIO_CANCEL_SIGNAL:
      {
        ...
        // SignalNotify.disposed promises that by the time it fires the handler
        // no longer affects delivery — for the signal's last subscriber that
        // means the default disposition is already restored. Hoisting this send
        // above the teardown would falsify that promise.
        ev->flags = ASIO_DISPOSABLE;
        pony_asio_event_send(ev, ASIO_DISPOSABLE, 0);
        break;
      }
