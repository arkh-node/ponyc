...
The `signals` package's signal handling system has been replaced, implementing [RFC #220](https://github.com/ponylang/rfcs/pull/220).
...
- A signal arriving during runtime shutdown no longer races the runtime's teardown into freed memory: the runtime drops a signal that arrives while teardown is committing and restores default dispositions for still-registered handlers on its way out.
