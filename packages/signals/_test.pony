...
actor \nodoc\ Main is TestList
  ...
  fun tag tests(test: PonyTest) =>
    ...
    // The "signals" exclusion group runs in this registration order. The
    // disposition-restore test must stay before the subscriber-limit test:
    // both use SIGTERM, and the limit test's subscriber table drains
    // asynchronously after it completes (see both docstrings).
    test(_TestDispose)
...
class \nodoc\ iso _TestDisposeRestoresDefaultDisposition is UnitTest
  ...
  by the other tests in this package. Must run before the subscriber-limit
  test, which also uses SIGTERM — see its docstring and the registration
  list in Main.tests.
  """
...
class \nodoc\ iso _TestSubscriberLimit is UnitTest
  ...
  Uses SIGTERM: it is valid on every platform (including Windows). The
  disposition-restore test also uses SIGTERM and must run first — its
  probe needs the subscriber table empty, and this test's table drains
  asynchronously after it completes. The "signals" exclusion group runs
  in registration order, and the registration list in Main.tests keeps
  that order; any future SIGTERM test must either run before this one or
  tolerate residual occupancy.
  """
